...
@IBDesignable open class RangeSlider: UIControl {
  ...
  ///The color of the track bar within the selected range
    @IBInspectable open var trackHighlightTintColor: UIColor = UIColor(red: 62/255.0, green: 156/255.0, blue: 53/255.0, alpha: 1.0){
    didSet {
      track.setNeedsDisplay()
    }
  }
  ...
  @IBInspectable open var hideLabels: Bool = false {
    ...
    @IBInspectable open var labelBackgroundColor: UIColor = UIColor.clear {
        didSet {
            lowerLabel.backgroundColor = labelBackgroundColor.cgColor
            
            upperLabel.backgroundColor = labelBackgroundColor.cgColor
        }
    }
  ...
  required public init(coder: NSCoder) {
    ...
    var minLabelBackgroundView =  UIView()
    var maxLabelBackgroundView =  UIView()
  func addContentViews(){

      minLabelBackgroundView = UIView()
      minLabelBackgroundView.backgroundColor = UIColor(red: 231/255.0, green: 243/255.0, blue: 230/255.0, alpha: 1.0)
      minLabelBackgroundView.layer.cornerRadius = 3
         addSubview(minLabelBackgroundView)

      
         maxLabelBackgroundView = UIView()
      maxLabelBackgroundView.backgroundColor = UIColor(red: 231/255.0, green: 243/255.0, blue: 230/255.0, alpha: 1.0)
      maxLabelBackgroundView.layer.cornerRadius = 3
  ...
  open func updateTrackLayerFrameAndKnobPositions() {
    ...
    let newTrackDy = (frame.height - TrackThickness) / 10
    track.frame = CGRect(x: 0, y: newTrackDy, width: frame.width, height: 6)
    track.setNeedsDisplay()
    ...
    open func getLabelText(forValue value: Double) -> String {
        ...
        guard let labelText = numberFormatter.string(from: NSNumber(value: value)) else {
            return ""
        }
  ...
  open func updateLabelText() {
    ...
    upperLabel.foregroundColor = labelColor.cgColor
      ...
      lowerLabel.cornerRadius = 3
      upperLabel.cornerRadius = 3
      
      lowerLabel.backgroundColor = labelBackgroundColor.cgColor  // Set the background color
          upperLabel.backgroundColor = labelBackgroundColor.cgColor  // Set the background color
  ...
  ///Updates the labels positions above the knobs.
    open func updateLabelPositions() {
        ...
        // Center labels within their background views
        minLabelBackgroundView.center = lowerLabel.position
        maxLabelBackgroundView.center = upperLabel.position

        // Set the frame for the min label background view
        minLabelBackgroundView.frame = CGRect(
            x: lowerLabel.frame.origin.x - 2,  // Adjust the padding as needed
            y: lowerLabel.frame.origin.y - 5,   // Adjust the padding as needed
            width: lowerLabel.frame.width + 10, // Adjust the padding as needed
            height: lowerLabel.frame.height + 10 // Adjust the padding as needed
        )

        // Set the frame for the max label background view
        maxLabelBackgroundView.frame = CGRect(
            x: upperLabel.frame.origin.x - 2,  // Adjust the padding as needed
            y: upperLabel.frame.origin.y - 5,   // Adjust the padding as needed
            width: upperLabel.frame.width + 10, // Adjust the padding as needed
            height: upperLabel.frame.height + 10 // Adjust the padding as needed
        )
    }
